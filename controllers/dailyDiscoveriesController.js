...
exports.getDailyDiscoveries = async (req, res) => {
    try {
        ...
        while (responseProducts.length < MIN_REQUIRED && attempts < MAX_ATTEMPTS) {
            ...
            const enrichedProducts = await Promise.all(matchedProducts.map(async (product) => {
                ...
                // For daily discoveries, we'll show products with significant price differences
                if (Math.abs(diff.percentageDifference) < 20) return null;
        ...
        return res.json({
            ...
            daily_discoveries: finalResults
        });

    } catch (err) {
        console.error("Error in getDailyDiscoveries:", err);
